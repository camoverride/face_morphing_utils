...
if __name__ == "__main__":
    ...
    # Get the landmarks.
    face_landmarks_1 = get_face_landmarks(image_1) # type: ignore
    face_landmarks_2 = get_face_landmarks(image_2) # type: ignore

    # Get additional landmarks around the edges of the image.
    additional_landmarks_1 = get_additional_landmarks(
        image_height=image_1.shape[0], # type: ignore
        image_width=image_1.shape[1]) # type: ignore
    all_landmarks_1 = face_landmarks_1 + additional_landmarks_1

    additional_landmarks_2 = get_additional_landmarks(
        image_height=image_2.shape[0], # type: ignore
        image_width=image_2.shape[1]) # type: ignore
    all_landmarks_2 = face_landmarks_2 + additional_landmarks_2

    # Draw landmarks on both images.
    image_1_copy = image_1.copy() # type: ignore
    for landmark in all_landmarks_1:
        x, y = landmark
        cv2.circle(image_1_copy, (x, y), radius=3, color=(0, 255, 0), thickness=-1) # type: ignore

    image_2_copy = image_2.copy() # type: ignore
    for landmark in all_landmarks_2:
        x, y = landmark
        cv2.circle(image_2_copy, (x, y), radius=3, color=(0, 255, 0), thickness=-1) # type: ignore

    # Show both images with landmarks.
    cv2.imshow("(1) Landmarks", image_1_copy) # type: ignore
    cv2.waitKey(0)
    cv2.destroyAllWindows()

    cv2.imshow("(2) Landmarks", image_2_copy) # type: ignore
    cv2.waitKey(0)
    cv2.destroyAllWindows()
    ...
    morphed_face_1 = morph_align_face(
        source_face=image_1, # type: ignore
    ...
    morphed_face_2 = morph_align_face(
        source_face=image_2, # type: ignore
    ...
    for alpha in [1, 0.9, 0.8, 0.7, 0.6, 0.5, 0.4, 0.3, 0.2, 0.1]:
        new_landmarks = compute_intermediate_landmarks(
            landmarks_1=all_landmarks_1, # type: ignore
            landmarks_2=all_landmarks_2, # type: ignore
            alpha=alpha)

        # Morph face 1 onto face 2.
        morphed_face_x = morph_align_face(
            source_face=image_1, # type: ignore
            target_face_all_landmarks=new_landmarks, # type: ignore
            triangulation_indexes=None) # type: ignore
